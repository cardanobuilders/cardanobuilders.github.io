...
このセクションでは、マルチvalidatorアーキテクチャをWeb2パターンに対応付け、3つのvalidatorがどのように連携して一意の自動インクリメントNFTをミントするかを説明します。
...
│   ├── components/         # オラクルセットアップ + ミンティングUIのReactコンポーネント
│   └── lib/                # コントラクトヘルパー、blueprintの読み込み、Meshユーティリティ
├── aiken-workspace/        # オンチェーンスマートコントラクトコード
│   ├── lib/
│   │   └── plutus-nft/
│   │       └── types.ak    # OracleDatum、MintPolarity、OracleRedeemer
│   ├── validators/
│   │   ├── oracle_nft.ak   # ワンタイムミンティングポリシー
│   │   ├── oracle.ak       # 状態管理validator
│   │   └── plutus_nft.ak   # NFTミンティングvalidator
...
プロジェクトはvestingレッスンと同じ2部構成です：`aiken-workspace/`にオンチェーンvalidatorがあり、`src/`がNextJSフロントエンドです。決定的な違いは、このプロジェクトでは1つの単独validatorではなく3つのvalidatorが連携して動作することです。NextJSアプリは1つのアトミックな操作で3つすべてに触れるマルチvalidatorトランザクションをオーケストレーションします。

### 3つのvalidatorアーキテクチャ

このシステムは根本的な問題を解決します：誰でもいつでもミンティングトランザクションを送信できる状況で、どのようにして一意で連番のNFTを保証するのか？ 答えはvalidatorのトリオによる連携です。

**1. オラクルNFT Validator（`oracle_nft.ak`）** -- ワンタイムミンティングポリシー。ミンティング中に特定のUTXOを消費することを要求し、オラクルトークンが1つだけ存在することを保証します。そのUTXOが消費されると、トークンは二度と再ミントできません。これはブートストラップステップです。

**2. オラクルValidator（`oracle.ak`）** -- 状態管理者。オラクルNFTをスクリプトアドレスに保持し、現在のNFTインデックス（`count`）、ミンティング価格（`lovelace_price`）、管理者アドレス（`fee_address`）を含むdatumと一緒に保管します。NFTがミントされるたびに、オラクルvalidatorはカウントが正確に1増加し、手数料が支払われたことを強制します。

**3. Plutus NFT Validator（`plutus_nft.ak`）** -- 実際のNFTのミンティングポリシー。オラクルの現在のdatumを読み取って次のインデックスを決定し、トークン名を`CollectionName (index)`として構築し、その名前のトークンが正確に1つミントされることを強制します。

### マルチvalidatorの相互作用フロー

```mermaid
sequenceDiagram
    participant User as ユーザー
    participant App as NextJSアプリ（Mesh SDK）
    participant OracleNFT as オラクルNFTポリシー
    participant Oracle as オラクルValidator
    participant PlutusNFT as Plutus NFTポリシー

    Note over OracleNFT,Oracle: ワンタイムセットアップ（管理者が一度だけ実施）
    App->>OracleNFT: オラクルトークンをミント（paramUtxoを消費）
    App->>Oracle: オラクルトークンをdatum { count: 0, price, admin }でロック

    Note over User,PlutusNFT: 各NFTミント（繰り返し可能）
    User->>App: ミントをリクエスト
    App->>Oracle: オラクルUTXOを読み取り（現在のcountを取得）
    App->>Oracle: オラクルUTXOを消費（redeemer: MintPlutusNFT）
    Oracle->>Oracle: バリデーション：カウント増加、<br/>手数料支払い済み、出力値がクリーン
    App->>Oracle: { count + 1 }でオラクルUTXOを出力
    App->>PlutusNFT: "Collection (count)"というトークンをミント
    PlutusNFT->>PlutusNFT: バリデーション：オラクル入力が存在、<br/>トークン名がインデックスと一致
    App->>User: NFTが配信、オラクル状態が更新
```

これらすべてが単一のアトミックトランザクションで発生します。すべてのvalidatorが合格してトランザクション全体が成功するか、何も変更されないかのいずれかです。オラクルが更新されたがNFTがミントされていないという中間状態は存在しません。

### Web2との対応

自動インクリメントID、一意制約、行レベルロックを持つシステムを構築した経験がある方は、ここで使われているパターンを既に理解しています。

| Cardanoの概念 | Web2の対応物 | 役割 |
|---|---|---|
| **オラクルvalidator** | 自動インクリメント主キーを持つデータベーステーブル | 次に利用可能なインデックスの唯一の情報源を維持します |
| **ワンタイムミント（オラクルNFT）** | `UNIQUE`制約 / 設定テーブルのシングルトン行 | オラクルが正確に1つだけ存在することを保証 -- 重複は不可能 |
| **ステートスレッドトークン** | 行レベルロック（`SELECT ... FOR UPDATE`） | オラクルNFTがインプットとアウトプットの両方に存在する必要があり、同時に状態を変更できるトランザクションが1つだけであることを保証します |
| **`OracleDatum`** | データベースレコード：`{ id SERIAL, price INT, admin_address TEXT }` | カウンター、価格、管理者を格納するオンチェーン状態 -- 状態テーブルの行に相当 |
| **`count`フィールド** | 自動インクリメントカラム（`SERIAL` / `AUTO_INCREMENT`） | 次のNFTインデックスを追跡し、各ミントでアトミックにインクリメント |
| **`is_output_value_clean`チェック** | 入力サニタイズ / SQLインジェクション防止 | 攻撃者がオラクルUTXOに余分なトークンを詰め込むことを防止します。これによりトランザクションが肥大化し、オラクルが消費不可能になる可能性があります -- 不正な入力によるサービス拒否のブロックチェーン版 |
| **`StopOracle` redeemer** | 認証チェック付きの管理者`DELETE`エンドポイント | 管理者がオラクルNFTをバーンしてミンティングシステムをシャットダウンできるようにし、署名（認証済みAPIコールのようなもの）を要求します |
| **単一アトミックトランザクション** | `BEGIN` / `COMMIT`を持つデータベーストランザクション | 3つすべてのvalidatorが1つのトランザクションで実行 -- いずれかのチェックが失敗するとすべてロールバック |

Web2からの最大の概念的転換：リクエストをシリアライズする中央データベースサーバーが存在しません。代わりに、UTXOモデルが自然な同時実行制御を提供します。オラクルUTXOは一度しか消費できないため、2つの同時ミントリクエストは競合し、一方が成功してもう一方は更新された状態で再試行する必要があります。これはリレーショナルデータベースのバージョンカラムによる楽観的同時実行制御に類似しています。
