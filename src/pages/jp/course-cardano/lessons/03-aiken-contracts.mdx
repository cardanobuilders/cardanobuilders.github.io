...
> ソースコード: [GitHub](https://github.com/cardanobuilders/cardanobuilders.github.io/tree/main/codes/course-cardano/03-aiken-contracts)
...
このセクションでは、レッスンのソースコードリポジトリのガイドツアーを提供し、上記のAikenの概念をクローンしてローカルでビルドできるプロジェクト構成に結びつけます。

### プロジェクト構成

```
03-aiken-contracts/
├── aiken-workspace/          # メインのAikenプロジェクトフォルダ（このレッスンで使用）
│   └── validators/
│       ├── mint.ak           # ミンティングvalidator（常に成功 + パラメータ化）
│       ├── spend.ak          # スペンディングvalidator（oracle NFTパターン）
│       └── withdraw.ak       # 引き出しvalidator（常に成功）
├── mesh/                     # オフチェーンコードフォルダ（このレッスンでは使用しません）
└── package.json
```

**`aiken-workspace/`**にはオンチェーンvalidatorが含まれています。Cardanoブロックチェーン自体で実行されるコードです。これを「バックエンドロジック」フォルダと考えてください。各`.ak`ファイルは、トランザクションバリデーション時にネットワークが実行するPlutusスクリプトにコンパイルされます。

**`mesh/`**にはトランザクションを構築・送信するオフチェーンTypeScriptコードが含まれています。validatorと通信するフロントエンドまたはAPIクライアントに相当します。このフォルダは現在のレッスンでは使用されませんが、Webアプリケーションをコントラクトに接続する後のレッスンで関連します。

Web2のバックグラウンドがある方にとっては、データベース制約用の`contracts/`フォルダとAPIサーバー用の`client/`フォルダがあるのと同様の分割です。

### Validatorの種類とトランザクショントリガー

3つの`.ak`ファイルは、Cardanoで最も一般的な3つのvalidatorタイプに対応しています。各タイプは異なるトランザクション条件でトリガーされます:

```mermaid
flowchart TD
    TX["送信されたトランザクション"]
    TX -->|"このポリシーの下でトークンを\nミントまたはバーン"| MINT["mint.ak\nミンティングvalidator"]
    TX -->|"スクリプトアドレスに\nロックされたUTXOを消費"| SPEND["spend.ak\nスペンディングvalidator"]
    TX -->|"スクリプトステークアドレスから\nリワードを引き出し"| WITHDRAW["withdraw.ak\n引き出しvalidator"]

    MINT -->|"True/Falseを返す"| RESULT["トランザクションは有効？"]
    SPEND -->|"True/Falseを返す"| RESULT
    WITHDRAW -->|"True/Falseを返す"| RESULT
```

- **`mint.ak`**はvalidatorのPolicy IDの下でトークンをミントまたはバーンするトランザクション時に実行されます。「常に成功」ポリシーから始まり、オーナー署名とデッドラインの要求へと進化します。
- **`spend.ak`**はvalidatorのスクリプトアドレスにあるUTXOを消費しようとするトランザクション時に実行されます。この例ではoracle NFTパターンを使用し、消費を許可する前に特定のリファレンスインプットが存在することを確認します。
- **`withdraw.ak`**はvalidatorのリワードアカウントからステーキングリワードを引き出すトランザクション時に実行されます。ここでの「常に成功」バージョンはスキャフォールドです。レッスン5では、引き出しvalidatorが効率的なマルチUTXOバリデーションにどう使われるかを示します。

### Web2との対応

REST APIやサーバーサイドアプリケーションの構築に慣れている方は、以下のマッピングがメンタルモデルの橋渡しになるでしょう:

| Cardanoの概念 | Web2の対応物 | 説明 |
|---|---|---|
| **Validator** | ルートミドルウェア / 認証ガード | 許可または拒否を返す純粋関数です。状態を直接変更せず、トランザクションが許可されるかどうかのみを判定します。 |
| **Datum** | データベースレコード / 行 | スクリプトアドレスのUTXOに添付されるデータです。資金を「ロック」する際にdatumを添付するのは、テーブルに行を書き込むのと同じです。 |
| **Redeemer** | APIリクエストボディ | ユーザーがvalidatorとやり取りする際に提供するデータです。validatorはredeemerを読んで、どのコードパスを実行するか（例：`MintToken` vs `BurnToken`）を決定します。 |
| **Policy ID** | APIキースコープ / 権限セット | ミンティングポリシーを識別する一意のハッシュです。異なるポリシーの下でミントされたトークンは区別され、異なるAPIキーが異なるリソースへのアクセスを許可するのと同様です。 |
| **Transaction** | HTTPリクエスト | validatorが検査するインプット、アウトプット、署名、メタデータの完全なバンドルで、ミドルウェアが受け取る完全なリクエストオブジェクトに相当します。 |

### 重要なポイント

すべてのAiken validatorは純粋関数です。トランザクションコンテキストを受け取り、`True`または`False`を返します。validator内に副作用、データベース書き込み、ネットワーク呼び出しはありません。ブロックチェーンランタイムは、validatorが承認した場合にのみ状態変更を処理します。これはWeb2バックエンドとは根本的に異なり、Web2ではルートハンドラがバリデーションと状態変更の両方を行います。

## ソースコード

このレッスンのソースコードは[GitHub](https://github.com/cardanobuilders/cardanobuilders.github.io/tree/main/codes/course-cardano/03-aiken-contracts)で公開されています。
