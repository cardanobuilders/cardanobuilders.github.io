...
After building Aiken contracts, it's important to test them to ensure they behave as expected. In this lesson, we will cover:

- Preparing a more complex contract for testing
- Building mock transactions in Aiken & running tests

## Prepare a more complex contract

We will continue using the withdrawal contract from the previous lesson. We will make the withdrawal contract do the following:

1. There are 2 potential user actions: `ContinueCounting` or `StopCounting`.
2. When the user chooses `ContinueCounting`, the contract will:
    - Verify the transaction is signed by the app owner.
    - Verify the app is not expired (using a POSIX timestamp).
    - Verify the state thread token is carried forward to the output.
    - Increment the count in the state thread token's datum by 1.
3. When the user chooses `StopCounting`, the contract will:
    - Verify the transaction is signed by the app owner.
    - Verify the state thread token is burned (not carried forward to any output).
...
We continue completing the contract:
...
Try to write tests for `StopCounting` action. You can refer to the `ContinueCounting` tests for guidance. The suggested answer can be found in the code example.