...
Here, we added a parameter `owner_vkey` to the validator, which can be perceived as a constant value that is set after the script is compiled and applied in client side (like with MeshJS).

The parameter is the public key hash of the owner who is allowed to mint or burn assets under this policy. In the `mint` function, we check if the transaction is signed by the `owner_vkey` using the `key_signed` function from [vodka](https://github.com/sidan-lab/vodka), which is a library provides on-chain utility functions and unit testing framework. To learn more about `vodka`, there is [hosted documetation](https://sidan-lab.github.io/vodka/).
