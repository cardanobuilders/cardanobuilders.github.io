---
title: Hello World
description: In this lesson, we will install Mesh SDK and learn how to send assets using the Mesh wallet.
asIndexPage: true
...
Welcome to the first lesson of the Cardano Application Development Course! In this session, you'll set up the [Mesh SDK](https://meshjs.dev/) and, learn how to create wallet using `MeshWallet` and send assets using `MeshTxBuilder`.
...
- `brew` method to generate a new mnemonic phrase
...
- `networkId` to specify the network, 0 for preprod testnet
- `key` to specify the key type and mnemonic phrases
- `getChangeAddress` method to get the wallet address

### Run the code

Here is the source code, you can create a new file `mnemonic.ts` and copy the code into it:
...
Here is the source code, you can create a new file `send-lovelace.ts` and copy the code into it:
...
Create a transaction that send multiple assets to multiple addresses.
