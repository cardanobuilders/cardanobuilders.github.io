---
title: Hello World
description: In this lesson, we will install Mesh SDK and learn how to send lovelace using the Mesh wallet.
asIndexPage: true
...
Welcome to the first lesson of the Cardano Application Development Course! In this lesson, we will install the [Mesh SDK](https://meshjs.dev/) and learn how to send lovelace using the Mesh wallet. This is a simple "Hello World" example that will help you get started with building applications on Cardano.

## System setup

Before we begin, lets prepare our system for development. We will be using Node.js and TypeScript for this course, so make sure you have them installed on your machine.
...
## Get new wallet and its address
...
To create a new wallet, we will use the `brew` method to generate a new mnemonic phrase.
...
Then, we will initialize the wallet with the generated mnemonic and get the wallet address.
...
Here is the full source code, you can create a new file `mnemonic.ts` and copy the code into it:
...
And update the `package.json` file to add a script to run the code:
...
### Run the code
Now, you can run the code using the following command:
...
To get some lovelace for testing, you can use the [Cardano Preprod Testnet Faucet](https://docs.cardano.org/cardano-testnets/tools/faucet). Just paste your wallet address and click on the "Request funds" button. You should receive some lovelace in your wallet shortly.
...
In order to create transactions, we need to use APIs to get UTXOs from the network. For this, we will use Blockfrost to get UTXOs and submit transactions. You can sign up for a free account and get your API key [here](https://blockfrost.io/).
...
- `getChangeAddress` to get the change address
...
- `txOut` method to add the recipient address and amount
- `changeAddress` method to set the change address
- `selectUtxosFrom` method to provide wallet UTXOs into the transaction as inputs
- `complete` method to create the transaction.
...
Here is the full source code, you can create a new file `send-lovelace.ts` and copy the code into it:
...
You need to replace with your API key and mnemonic phrases. You can use the mnemonic phrases generated in the previous step.

You can also replace the recipient address with your own address.
